...
<xsl:stylesheet exclude-result-prefixes="tei jc" version="2.0" xmlns="http://www.tei-c.org/ns/1.0" xmlns:jc="http://james.blushingbunny.net/ns.html" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xpath-default-namespace="http://www.tei-c.org/ns/1.0">
  <!-- 
  ...
  <xsl:variable name="path">
    <xsl:value-of select="concat('file:///home/jamesc/Dropbox/stuff/Desktop/Work/projects/Bodleian-TEI-Catalogue-Consolidation/working/tolkien-xml/working/?select=', $files, ';on-error=warning;recurse=', $recurse)"/>
  </xsl:variable>
  ...
  <xsl:template name="main">
    ...
    <xsl:for-each select="$doc">
      ...
      <xsl:message> Base URI: <xsl:value-of select="$baseURI"/> Folder: <xsl:value-of select="$folder"/> Old Filename: <xsl:value-of select="$filename"/> New ID: <xsl:value-of select="$msID"/>
      </xsl:message>
      <!-- Create the (hard coded) output file name -->
      <xsl:variable name="outputFilename" select="concat('file:/home/jamesc/Dropbox/stuff/Desktop/Work/projects/Bodleian-TEI-Catalogue-Consolidation/working/tolkien-xml/new/', $folder, '/', $msID, '.xml')"/>
      <!-- create output file -->
      <xsl:result-document href="{$outputFilename}" indent="yes" method="xml">
...
msPart/altIdentifier needs to be changed to msIdentifier
  ...
  <xsl:template match="msItem">
    ...
    <xsl:variable name="msItemID">
      ...
      <xsl:choose>
        <xsl:when test="parent::msItem/parent::msItem/parent::msItem/parent::msItem/parent::msContents"> -item<xsl:value-of select="count(parent::msItem/parent::msItem/parent::msItem/parent::msItem/preceding-sibling::msItem) + 1"/> -item<xsl:value-of select="count(parent::msItem/parent::msItem/parent::msItem/preceding-sibling::msItem) + 1"/> -item<xsl:value-of select="count(parent::msItem/parent::msItem/preceding-sibling::msItem) + 1"/> -item<xsl:value-of select="count(parent::msItem/preceding-sibling::msItem) + 1"/> -item<xsl:value-of select="count(preceding-sibling::msItem) + 1"/>
        </xsl:when>
        <xsl:when test="parent::msItem/parent::msItem/parent::msItem/parent::msContents"> -item<xsl:value-of select="count(parent::msItem/parent::msItem/parent::msItem/preceding-sibling::msItem) + 1"/> -item<xsl:value-of select="count(parent::msItem/parent::msItem/preceding-sibling::msItem) + 1"/> -item<xsl:value-of select="count(parent::msItem/preceding-sibling::msItem) + 1"/> -item<xsl:value-of select="count(preceding-sibling::msItem) + 1"/>
        </xsl:when>
        <xsl:when test="parent::msItem/parent::msItem/parent::msContents"> -item<xsl:value-of select="count(parent::msItem/parent::msItem/preceding-sibling::msItem) + 1"/> -item<xsl:value-of select="count(parent::msItem/preceding-sibling::msItem) + 1"/> -item<xsl:value-of select="count(preceding-sibling::msItem) + 1"/>
        </xsl:when>
        <xsl:when test="parent::msItem/parent::msContents"> -item<xsl:value-of select="count(parent::msItem/preceding-sibling::msItem) + 1"/> -item<xsl:value-of select="count(preceding-sibling::msItem) + 1"/>
        </xsl:when>
  ...
  <xsl:function as="item()*" name="jc:splitAltIdentifier">
    <xsl:param as="item()" name="altIdentifier"/>
    <xsl:choose>
      <xsl:when test="$altIdentifier/idno[@type = 'SCN' and not(contains(., 'Not in SC')) and contains(., ',')] | $altIdentifier/idno[@type = 'TM']">
  ...
  <xsl:function as="item()" name="jc:normalizeLang">
    <xsl:param as="item()" name="lang"/>
