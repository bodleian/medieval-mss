   ...
   <body>
      ...
      <table
         ...
         </tr>
         {
            ...
            return
               if (
               (some $a in $authorkeys1
                  satisfies not($a = $authorkeys2)) or
               (some $a in $authorkeys2
                  satisfies not($a = $authorkeys1)) or
               (some $l in $langs1
                  satisfies not($l = $langs2)) or
               (some $l in $langs2
                  satisfies not($l = $langs1))
               ) then
                  <tr>
                     <td>
                        {
                           if (count($msitems) gt 0) then
                              <a
                                 href="{$websitecatalog}{$workid}">{$worktitle}</a>
                           else
                              $worktitle
                        }
                     </td>
                     <td>{
                           for $a in $authorkeys1
                           return
                              if (not($a = $authorkeys2)) then
                                 (<a
                                    href="{$websitecatalog}{$a}">{distinct-values($authors1[@key/data() = $a]/text())}</a>, <br/>)
                              else
                                 ()
                        }</td>
                     <td>{
                        ...
                        }</td>
                     {
                        if (count($msitems) eq 0) then
                           <td
                              colspan="2"
                              align="center">Work not found in manuscript descriptions</td>
                        else
                           <td>{
                                 for $a in $authorkeys2
                                 return
                                    if (not($a = $authorkeys1)) then
                                       (<a
                                          href="{$websitecatalog}{$a}">{distinct-values($authors2[@key/data() = $a]/text())}</a>, <br/>)
                                    else
                                       ()
                              }</td>
                     }
