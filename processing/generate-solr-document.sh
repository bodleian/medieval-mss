...
# Start log file
LOGFILE="solr/$3.log"
echo "Processing TEI files in collections folder using $1 on $(date +"%Y-%m-%d %H:%M:%S") to be sent to $4 for reindexing." > $LOGFILE

# Run XQuery to build Solr XML index files
echo "Generating XML file to send to Solr for $3s..."
java -Xmx1G -Xms1G -cp "saxon/saxon9he.jar" net.sf.saxon.Query -q:$1 1> solr/$2 2>> $LOGFILE
if [ $? -gt 0 ]; then
    echo "XQuery failed. Please raise an issue on GitHub, attaching $LOGFILE. Reindexing of $3 cancelled."
    exit 1;
fi
...
if [ $errors -gt 0 ]; then
    echo "There are $errors error messages in $LOGFILE. Reindexing of $3 cancelled."
    exit 1;
fi
...
if [ $warnings -gt 0 ] || [ $infos -gt 0 ]; then
    ...
    if [ ! "$5" == "force" ] && [ ! "$5" == "noindex" ]; then
        while true; do
            read -p "Do you wish to reindex? [Yes|No|Quit|View] " answer
            case $answer in
                [Yy]|YES|Yes|yes ) break;;
                [Nn]|NO|No|no ) echo "Reindexing of $3 cancelled. Proceeding to next index."; exit 0;;
                [Qq]|QUIT|Quit|quit ) echo "Reindexing of $3 cancelled. Abandoning all further indexing."; exit 1;;
...
if [ ! "$5" == "noindex" ]; then
    ...
    if [ $? -gt 0 ]; then
        ...
        echo "Reindexing $3 in Solr..."
        curl -fsS "http://${4}:8983/solr/medieval-mss/update?commit=true" --data-binary @solr/$2 -H "Content-Type: text/xml" 1>> $LOGFILE 2>> $LOGFILE
        if [ $? -eq 0 ]; then
            echo "Reindexing of $3 finished. Please check the web site for expected changes."
            exit 0;
        else
            echo "Reindexing of $3 failed. The web site will have no $3s. If this is on production, raise an urgent issue on GitHub, attaching $LOGFILE."
            exit 1;
        fi
    fi
else
