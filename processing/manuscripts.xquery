   ...
   if (count($keys) gt 0) then
      ...
      return
         if (count($countries) gt 0) then
            ...
            return
               <field
                  name="{$solrfield}">{$name}</field>
            ,
            if (count($countries) gt 1) then
               <field
                  name="{$solrfield}">Multiple Origins</field>
            else
               ()
            )
         else
            <field
               name="{$solrfield}">[MISSING]</field>
   else
      ()
};
...
declare function local:buildSummary($msdescorpart as element()) as xs:string
{
   ...
   let $worktitles := distinct-values(for $t in $msdescorpart//tei:msItem/tei:title[1]/normalize-space()
   return
      if (ends-with($t, '.')) then
                ...
                :)
                  return
                     <doc>
                        ...
                        <field
                           name="filename_s">{substring-after(base-uri($ms), 'collections/')}</field>
