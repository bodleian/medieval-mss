...
return
   <instance>
         ...
         }</link>
      {
         ...
         return
            <translator>{$translatorid}</translator>
      }
      {
         for $instanceid in $instance/parent::tei:msItem/@xml:id
         return
...
<add>
   ...
   }
   {
      ...
      let $title := if ($work/tei:title[@type = 'uniform']) then
         normalize-space($work/tei:title[@type = 'uniform'][1]/string())
      else
         normalize-space($work/tei:title[1]/string())
      let $variants := for $v in $work/tei:title[not(@type = 'uniform')]
      return
         normalize-space($v/string())
      let $extrefs := for $r in $work/tei:note[@type = 'links']//tei:item/tei:ref
      return
         concat($r/@target/data(), '|', bod:lookupAuthorityName(normalize-space($r/tei:title/string())))
      let $bibrefs := for $b in $work/tei:bibl
      return
         bod:italicizeTitles($b)
      let $repertories := for $n in $work/tei:bibl[@type = 'repertory']
      return
         normalize-space($n/string())
      let $notes := for $n in $work/tei:note[not(@type = ('links', 'shelfmark', 'language', 'subject'))]
      return
         ...
         if (count($instances) gt 0) then
            <doc>
               ...
               </field>
               {
                  ...
                  return
                     <field
                        name="wk_variant_sm">{$variant}</field>
               }
               {
                  let $lcvariants := for $variant in ($title, $variants)
                  return
                     lower-case($variant)
                  ...
                  return
                     if (not(lower-case($instancevariant) = $lcvariants)) then
                        <field
                           name="wk_variant_sm">{$instancevariant}</field>
                     else
                        ()
               }
               {
               ...
               }
               {
                  ...
                  return
                     <field
                        name="repertory_sm">{$repertory}</field>
               }
               {
               ...
               }
               {
                  ...
                  return
                     <field
                        name="wk_subjects_sm">{normalize-space($subject)}</field>
               }
               {
               ...
               }
               {
                  ...
                  let $authorids :=
                  if ($authorsinworksauthority) then
                     distinct-values($work/tei:author[not(@role)]/@key/data())
                  else
                     distinct-values(($instances/author/text(), $work/tei:author[not(@role)]/@key/data()))
                  ...
                  return
                     if (exists($linktext) and exists($personkeys[. = $authorid])) then
                        ...
                        return
                           <field
                              name="link_author_smni">{$link}</field>
                     else
               ...
               }
               {
                  ...
                  let $translatorids :=
                  if ($authorsinworksauthority) then
                     distinct-values($work/tei:author[@role = ('translator', 'trl')]/@key/data())
                  else
                     distinct-values(($instances/translator/text(), $work/tei:author[@role = ('translator', 'trl')]/@key/data()))
                  ...
                  return
                     if (exists($linktext) and exists($personkeys[. = $translatorid])) then
                        ...
                        return
                           <field
                              name="link_translator_smni">{$link}</field>
                     else
   ...
   }
   {
      ...
      return
         bod:logging('info', 'Work title without key attribute', $instancetitle)
   }
</add>