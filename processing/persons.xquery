...
let $roles := if ($instance/self::tei:author) then
   ('aut')
else
   tokenize($instance/@role/data(), ' ')
   ...
   <instance>
      ...
      }
      {
         if ($authorsinworksauthority) then
            ...
            if (some $role in $roles
               satisfies $role = ('author', 'aut') and not($instance/parent::tei:bibl)) then
               for $workid in distinct-values($instance/ancestor::tei:msItem[tei:title/@key][1]/tei:title/@key/tokenize(data(), ' '))
               return
                  <authored>{$workid}</authored>
            else
               if (some $role in $roles
                  satisfies $role = ('translator', 'trl') and not($instance/parent::tei:bibl)) then
                  for $workid in distinct-values($instance/ancestor::tei:msItem[tei:title/@key][1]/tei:title/@key/tokenize(data(), ' '))
                  return
                     <translated>{$workid}</translated>
               else
                  ()
      }
...
<add>
   ...
   }
   {
      ...
      let $name := if ($person/tei:persName[@type = 'display']) then
         normalize-space($person/tei:persName[@type = 'display'][1]/string())
      else
         normalize-space($person/tei:persName[1]/string())
      let $variants := for $variant in $person/tei:persName[not(@type = 'display')]
      return
         normalize-space($variant/string())
      let $extrefs := for $ref in $person/tei:note[@type = 'links']//tei:item/tei:ref
      return
         concat($ref/@target/data(), '|', bod:lookupAuthorityName(normalize-space($ref/tei:title/string())))
      let $extauths := distinct-values(for $ref in $person/tei:note[@type = 'links']//tei:item/tei:ref
      return
         normalize-space($ref/tei:title/string()))
      let $bibrefs := for $bibl in $person/tei:bibl
      return
         bod:italicizeTitles($bibl)
      let $notes := for $note in ($person/tei:note[not(@type = 'links')], $person/ancestor::tei:listPerson/tei:head/tei:note)
      return
         bod:italicizeTitles($note)
      ...
      let $isauthor := some $role in $instances/role/text()
         satisfies $role = ('author', 'aut')
      let $istranslator := some $role in $instances/role/text()
         satisfies $role = ('translator', 'trl')
         
         (: Output a Solr doc element :)
      return
         if (count($instances) gt 0) then
            <doc>
               <field
                  name="type">person</field>
               <field
                  name="pk">{$id}</field>
               <field
                  name="id">{$id}</field>
               ...
               }
               {
                  ...
                  return
                     <field
                        name="pp_variant_sm">{$variant}</field>
               }
               {
                  ...
                  return
                     if (not(lower-case($instancevariant) = $lcvariants)) then
                        <field
                           name="pp_variant_sm">{$instancevariant}</field>
                     else
                        ()
               }
               {
               ...
               }
               {
                  ...
                  if ($isauthor) then
                     let $workids :=
                     if ($authorsinworksauthority) then
                        distinct-values($worksauthority[tei:author[not(@role)]/@key = $id]/@xml:id)
                     else
                        distinct-values(($instances/authored/text(), $worksauthority[tei:author[not(@role)]/@key = $id]/@xml:id))
                     return
                        ...
                        return
                           if (exists($linktext) and exists($workkeys[. = $workid])) then
                              ...
                              return
                                 <field
                                    name="link_works_smni">{$link}</field>
                           else
               ...
               }
               {
                  ...
                  if ($istranslator) then
                     ...
                     then
                        distinct-values($worksauthority[tei:author[@role = ('trl', 'translator')]/@key = $id]/@xml:id)
                     else
                        distinct-values(($instances/translated/text(), $worksauthority[tei:author[@role = ('trl', 'translator')]/@key = $id]/@xml:id))
                     return
                        for $workid in $workids
                        let $url := concat("/catalog/", $workid)
                        let $linktext := ($worksauthority[@xml:id = $workid]/tei:title[@type = 'uniform'][1])[1]
                        ...
                        return
                           if (exists($linktext) and exists($workkeys[. = $workid])) then
                              ...
                              return
                                 <field
                                    name="link_translations_smni">{$link}</field>
                           else
   ...
   }
   {
      ...
      return
         bod:logging('info', 'Person without key attribute', $instancename)
   }
</add>